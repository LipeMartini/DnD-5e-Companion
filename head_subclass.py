"""
Subclasses (Arqu├®tipos) para D&D 5e
Cada classe tem suas pr├│prias subclasses que s├úo escolhidas em n├¡veis espec├¡ficos
"""
...
class SubclassFeature:
    """Representa uma feature espec├¡fica de uma subclasse"""
    name: str
    level: int
    description: str
...
BERSERKER = Subclass(
    ...
    description="For some barbarians, rage is a means to an end ÔÇö that end being violence. The Path of the Berserker is a path of untrammeled fury, slick with blood.",
    selection_level=3,
    features=[
        SubclassFeature(
            name="Frenzy",
...
# Dicion├írio de subclasses por classe
...
LIFE_DOMAIN = Subclass(
    name="Life Domain",
    class_name="Cleric",
    description="The Life domain focuses on the vibrant positive energyÔÇöone of the fundamental forces of the universeÔÇöthat sustains all life. The gods of life promote vitality and health through healing the sick and wounded, caring for those in need, and driving away the forces of death and undeath.",
    selection_level=1,
    features=[
        SubclassFeature(
            name="Bonus Proficiency",
...
CIRCLE_OF_THE_LAND = Subclass(
    ...
    features=[
        ...
        SubclassFeature(
            name="Circle Spells",
            level=3,
            description="Your mystical connection to the land infuses you with the ability to cast certain spells. You gain access to circle spells connected to the land where you became a druid. Choose that landÔÇöarctic, coast, desert, forest, grassland, mountain, swamp, or UnderdarkÔÇöand consult the associated list of spells. Once you gain access to a circle spell, you always have it prepared, and it doesn't count against the number of spells you can prepare each day."
        ),
        SubclassFeature(
            name="Land's Stride",
            level=6,
...
WAY_OF_THE_OPEN_HAND = Subclass(
    ...
    features=[
        SubclassFeature(
            ...
            level=3,
            description="You can manipulate your enemy's ki when you harness your own. Whenever you hit a creature with one of the attacks granted by your Flurry of Blows, you can impose one of the following effects on that target:\nÔÇó It must succeed on a Dexterity saving throw or be knocked prone.\nÔÇó It must make a Strength saving throw. If it fails, you can push it up to 15 feet away from you.\nÔÇó It can't take reactions until the end of your next turn."
        ),
        SubclassFeature(
...
OATH_OF_VENGEANCE = Subclass(
    ...
    class_name="Paladin",
    description="The Oath of Vengeance is a solemn commitment to punish those who have committed a grievous sin. When evil forces slaughter helpless villagers, when an entire people turns against the will of the gods, when a thieves' guild grows too violent and powerful, when a dragon rampages through the countrysideÔÇöat times like these, paladins arise and swear an Oath of Vengeance to set right that which has gone wrong.",
    selection_level=3,
    features=[
        SubclassFeature(
            name="Oath Spells",
        ...
        SubclassFeature(
            name="Avenging Angel",
            level=20,
            description="You can assume the form of an angelic avenger. Using your action, you undergo a transformation. For 1 minute, you gain the following benefits:\nÔÇó Wings sprout from your back and grant you a flying speed of 60 feet.\nÔÇó You emanate an aura of menace in a 30-foot radius. The first time any enemy creature enters the aura or starts its turn there during a battle, the creature must succeed on a Wisdom saving throw or become frightened of you for 1 minute or until it takes any damage. Attack rolls against the frightened creature have advantage.\nOnce you use this feature, you can't use it again until you finish a long rest."
        )
    ]
)
...
# Dicion├írio mestre de todas as subclasses
ALL_SUBCLASSES = {
...
class SubclassDatabase:
    ...
    def get_subclasses_for_class(class_name: str) -> Dict[str, Subclass]:
        """Retorna todas as subclasses dispon├¡veis para uma classe"""
        return ALL_SUBCLASSES.get(class_name, {})
    
    @staticmethod
    def get_subclass(class_name: str, subclass_name: str) -> Subclass:
        """Retorna uma subclasse espec├¡fica"""
        subclasses = ALL_SUBCLASSES.get(class_name, {})
        return subclasses.get(subclass_name)
    
    @staticmethod
    def get_selection_level(class_name: str) -> int:
        """Retorna o n├¡vel em que a classe escolhe sua subclasse"""
        # A maioria das classes escolhe no n├¡vel 3
        # Cleric e Warlock escolhem no n├¡vel 1
        # Sorcerer e Wizard escolhem no n├¡vel 1 (origem/escola)
        special_levels = {
            "Cleric": 1,
            "Warlock": 1,
            "Sorcerer": 1,
